...
def pair_trading_weights(
    ...
    *,
    ...
    z = (spread - spread.rolling(lookback, min_periods=lookback // 2).mean()) / (spread.rolling(lookback, min_periods=lookback // 2).std(ddof=0) + 1e-12)
...
def cross_sectional_rotation_weights(
    ...
    *,
    ...
    score = trend.rank(axis=1, pct=True) + (-rz).rank(axis=1, pct=True) + cycle_slope.rank(axis=1, pct=True) - 0.25 * noise.rank(axis=1, pct=True)
...
def detime_dual_ma_signals(prices: pd.DataFrame, features: dict[str, pd.DataFrame], *, fast: int = 20, slow: int = 100) -> tuple[pd.DataFrame, pd.DataFrame]:
    from .classic_indicators import dual_ma_signals
    entries, exits = dual_ma_signals(prices, fast=fast, slow=slow)
    entries = entries & (_align(features["trend_slope"], prices) > 0) & (_align(features["residual_abs_z"], prices) < 2.5)
    exits = exits | (_align(features["trend_slope"], prices) <= 0) | (_align(features["residual_abs_z"], prices) >= 2.5)
    return entries.fillna(False), exits.fillna(False)


def detime_macd_signals(prices: pd.DataFrame, features: dict[str, pd.DataFrame], *, fast: int = 12, slow: int = 26, signal: int = 9) -> tuple[pd.DataFrame, pd.DataFrame]:
    from .classic_indicators import macd
    parts = macd(_align(features["trend"], prices), fast=fast, slow=slow, signal=signal)
    entries = (parts["macd"] > parts["signal"]) & (_align(features["residual_abs_z"], prices) < 2.5)
    exits = (parts["macd"] < parts["signal"]) | (_align(features["residual_abs_z"], prices) >= 2.5)
    return entries.fillna(False), exits.fillna(False)


def detime_multi_ma_score_weights(prices: pd.DataFrame, features: dict[str, pd.DataFrame]) -> pd.DataFrame:
    score = _align(features["trend_slope"], prices).rank(axis=1, pct=True) + _align(features.get("cycle_slope", features.get("season_slope")), prices).rank(axis=1, pct=True) - _align(features["residual_z"], prices).rank(axis=1, pct=True)
    weights = pd.DataFrame(0.0, index=prices.index, columns=prices.columns)
    for dt, row in score.iterrows():
        valid = row.dropna()
        if not valid.empty:
            picks = valid.nlargest(min(3, len(valid))).index
